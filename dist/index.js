    ...
    return new Promise((resolve) => {
        vinted_api_1.default
            ...
            const items = res.items
                .sort((a, b) => new Date(b.created_at_ts).getTime() -
                new Date(a.created_at_ts).getTime())
                .filter((item) => !lastItemTimestamp ||
            ...
            for (let item of itemsToSend) {
                const embed = new discord_js_1.default.MessageEmbed()
                    ...
                    .addField("Note vendeur", `${getReputationStars(item.user.feedback_reputation)} (${(item.user.positive_feedback_count || 0) +
                    (item.user.neutral_feedback_count || 0) +
                    (item.user.negative_feedback_count || 0)})` || "vide", true)
            ...
            if (itemsToSend.length > 0) {
                console.log(`👕 ${itemsToSend.length} ${itemsToSend.length > 1 ? "new items found" : "new article found"} for research ${subscriptionData.id} !\n`);
            }
            resolve();
        })
            .catch((e) => {
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (!adminIDs.includes(interaction.user.id))
        return void interaction.reply(`:x: You do not have the rights to perform this action !`);
    switch (interaction.commandName) {
        ...
        case "unsubscribe": {
            ...
            interaction.reply(`:white_check_mark: Subscription successfully deleted !\n**URL**: <${subscription.url}>\n**Salon**: <#${subscription.channelId}>`);
            break;
        }
        case "abonnements": {
            const subscriptions = await (0, typeorm_1.getConnection)()
